...
def prepare_blocks_export(
    ...
    if isinstance(base_blocks, List):
        names = [block._name for block in base_blocks]
        ...
        for category, blocks in base_blocks.items():
            names = [block._name for block in blocks]
            duplicate_names = {name for name in names if names.count(name) > 1}
            if duplicate_names:
                raise ValueError(f"Duplicate blocks found in category '{category}': {duplicate_names}")
